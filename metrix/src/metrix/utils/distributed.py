...
class DistributedContext:
    """Runtime rank information derived from environment variables."""
...
def detect_distributed_context(env: Mapping[str, str] | None = None) -> DistributedContext:
    """Detect launcher/rank metadata from a process environment."""
...
def normalize_command_argv(command: str | Sequence[str]) -> list[str]:
    """
    Normalize command input into argv list.

    Accepts either a shell-like string or an explicit argv sequence.
    """
