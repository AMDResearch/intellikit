...
class Linex:
    ...
    def profile(
        ...
        cmd = [
            "rocprofv3",
            "--att",
            "--att-library-path", str(self.decoder_lib.parent),
            "--att-activity", str(self.activity),
            "--att-target-cu", str(self.target_cu),
            "--att-shader-engine-mask", self.shader_engine_mask,
            "-d", output_dir,
        ]
    ...
    def _load_ui_output(self, ui_output_dir: Path) -> None:
        ...
        if data['code'] is None or len(data['code']) == 0:
            raise ValueError("No code traced - target CU had no waves. Try larger workload or HSA_CU_MASK=0x1")
